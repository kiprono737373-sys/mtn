...
/* =========================================================================
   ⚠️  RENDER FREE TIER KEEP-ALIVE — READ THIS
   -------------------------------------------------------------------------
   Render suspends free web services after ~15 minutes of NO incoming
   HTTP traffic. When suspended, NO process is running, so the app CANNOT
   wake itself up. The only reliable fix is an EXTERNAL pinger.

   Set up ONE of these free services (2-minute setup):
     • https://cron-job.org  → ping https://YOUR-DOMAIN/ping every 10 min
     • https://uptimerobot.com → HTTP(s) monitor, interval 5 min

   The /ping endpoint below is intentionally ultra-lightweight so it
   doesn't burn CPU or bandwidth.
   ========================================================================= */

/* =========================================================================
   PROCESS-LEVEL CRASH GUARDS
   Keeps the worker from exiting on uncaught errors.
   ========================================================================= */
process.on('uncaughtException', (err) => {
  console.error('🔥 UNCAUGHT EXCEPTION:', err);
  // Do not exit — let the process continue serving traffic.
});
...
/* =========================================================================
   CLUSTER — auto-restart the worker if it ever dies
   ========================================================================= */
if (cluster.isPrimary) {
  ...
  function forkWorker() {
    ...
    worker.on('exit', (code, signal) => {
      console.error(`💀 Worker ${worker.process.pid} died (code=${code}, signal=${signal}). Restarting in 2s...`);
  ...
  // Also restart if the primary itself gets a signal
  process.on('SIGTERM', () => {
    console.log('📴 SIGTERM received by primary — shutting down cleanly.');
  ...
  // =====================================================================
  // WORKER — runs the actual Express server
  // =====================================================================
  startServer();
}
...
function startServer() {
  ...
  // ---------- KEEP-ALIVE (internal safety net) ----------
  async function pingSelf() {
    if (!DOMAIN) return;
    try {
      await axios.get(`${DOMAIN}/ping`, { timeout: 8000 });
  ...
  /* =========================================================
     ULTRA-LIGHT PING — used by external keep-alive services
     ========================================================= */
  ...
  // ---------- MANUAL RESTART (protected, useful for debugging) ----------
  // Requires RESTART_KEY env var. Hit: /restart?key=YOUR_KEY
  app.get('/restart', (req, res) => {
    ...
    res.send('Restarting worker...');
    console.log('♻️ Manual restart requested — exiting worker.');
    setTimeout(() => process.exit(1), 500); // exit → primary forks new worker
  });
  ...
  (async function bootstrap() {
    ...
    // Internal self-ping (helps while running, but cannot wake a sleeping instance)
